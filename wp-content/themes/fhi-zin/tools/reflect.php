<?php
	...
	*/
	...
	if(isset($_GET['merge_images']) && $_GET['merge_images']){
		$merge_images = true;
	}else{
		$merge_images = false;
	}
	
	if(isset($_GET['spacing']) && $_GET['spacing'] > 0){
	...
	if (isset($_GET['img']))
	{
        ...
        if (file_exists($source_image))
        {
            if ($cache)
            {
                $cache_dir = realpath('files/');//dirname($source_image);
	...
	if($grayscale){
		...
		for ($i=0; $i<$imgw; $i++)
		{
				for ($j=0; $j<$imgh; $j++)
				{
						...
						$val = imagecolorallocate($source, $g, $g, $g);
					   
						// set the gray value
					   
						imagesetpixel ($source, $i, $j, $val);
				}
		}	
	}
	...
	*/
	...
	if($merge_images){
		$finalimage = imagecreatetruecolor($width,$height + $new_height + $spacing);
		imagesavealpha($output, true);
		
		imagesavealpha($finalimage, true);
		imagealphablending($finalimage, false);
		
		imagecopymerge($finalimage,$source,0,0,0,0,$width,$height,100) ;
		imagecopymerge($finalimage,$output,0,$height + $spacing,0,0,$width,$new_height,100) ;
		$output = $finalimage;
	}
	...
	*/
	...
	if($spacing > 0 && $merge_images) imagefilledrectangle($output, 0, $height, $width, $height+$spacing, imagecolorallocatealpha($output, 127, 127, 127, 127));
	...
	*/
    ...
    for ($y = 0 ; $y <= $new_height; $y++)
    {
        ...
        imagefilledrectangle($output, 0, $merge_images?$y+$height+$spacing:$y, $width, $merge_images?$y+$height+$spacing:$y, imagecolorallocatealpha($output, $red, $green, $blue, $final_alpha));
    }
	...
	*/
	...
	else
	{
		...
		imagedestroy($output);
		if($finalimage) imagedestroy($finalimage);
		exit();
	}
?>