...
var bda = {
		...
		if(this.getHTTPRequest()){
			if(regions == 'undefined' && this.regions.length == 0){
				...
				if(this.primaryRegion == null){
					...
					this.primaryTween.onMotionFinished = function(){bda.triggerEventListeners('tween_complete',{});bda.transitionInProgress = false;};
	...
	ajaxRequest : function(url, parameters, requesttype, onlyNotify, urlToLoadWhenReady) {
		...
		if(url != bda.historyCurrentLocation && !onlyNotify){
			...
			bda.historyCurrentID = (new Date()).getTime();
		}
		...
		if(!onlyNotify) http_request.onreadystatechange = bda.ajaxReady;
	...
	outputPage : function(content){
		...
		this.setContent(primaryContent.toSource());
	...
	transitionHeight : function(to){
		...
		if(!this.isLoading) this.transitionInProgress = true;
